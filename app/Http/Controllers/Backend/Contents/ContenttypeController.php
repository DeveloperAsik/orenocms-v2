...
use App\Models\Entity\uac\Tbl_a_app_contents_types_c_en;
/**
 * Description of ContenttypeController
 *
 * @author 64146
 */
class ContenttypeController extends Controller {
    ...
    //put your code here
    protected $General;
    protected $Tbl_a_app_contents_types_c_en;
    protected $Converter;
    protected $Date;
    ...
    public function __construct(Request $request) {
        ...
        $this->Date = new Date();
        $this->Tbl_a_app_contents_types_c_en = new Tbl_a_app_contents_types_c_en();
    }
    ...
    public function __get_list_default($request) {
        ...
        $data = $this->Tbl_a_app_contents_types_c_en->__find($request, 'all', $params);
    ...
    public function __get_data_by_id($request) {
        ...
        if ($data['id'] == 0) {
            ...
            $dataDB = $this->Tbl_a_app_contents_types_c_en->__find($request, 'first', $params);
    ...
    public function __insert_default($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_a_app_contents_types_c_en->__find($request, 'first', $params);
            if (isset($response['data']) && !empty($response['data']) && $response['data'] != null) {
                ...
                $this->Tbl_a_app_contents_types_c_en->__insert($request, $insert);
    ...
    public function __insert_update_menu_tree($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            if ($data['parent_id'] == 0) {
                ...
                $parent_menu = $this->Tbl_a_app_contents_types_c_en->__find($request, 'first', $params);
                if (isset($parent_menu['data']) && !empty($parent_menu['data'])) {
                    $parent_id = $parent_menu['data']->id;
                    $new_level = (int) $parent_menu['data']->__level + 1;

                    $last_rank = $this->Tbl_a_app_contents_types_c_en->__fetch_last_rank($request, $parent_id);
                    $new_rank = 1;
            ...
            $response = $this->Tbl_a_app_contents_types_c_en->__insert($request, $param_insert);
            if ($response) {
                return $this->General->_set_response('json', ['code' => 200, 'message' => 'successfully insert data', 'valid' => true]);
            }
        }
    ...
    public function edit(Request $request, $params = null) {
        ...
        $menu = $this->Tbl_a_app_contents_types_c_en->__find($request, 'first', $params);
        $this->load_css([
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/bootstrap-select/bootstrap-select.min.css",
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/select2/select2.css",
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/jquery-multi-select/css/multi-select.css"
    ...
    public function update(Request $request, $params = null) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_a_app_contents_types_c_en->__update($request, $update_data, $paramsUpdate);
            if ($response) {
    ...
    public function delete(Request $request, $params = null) {
        if ($params != null) {
            ...
            if ($existData && $existData['data']) {
                ...
                $this->Tbl_a_app_contents_types_c_en->__insert($request, $insertUserGroupsBackup, 'mysql_bak');
                $deleteParams = [
                    'table_name' => 'tbl_a_uac_menus_p',
                    'conditions' => [
                        'keyword' => 'id',
