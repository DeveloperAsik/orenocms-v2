...
class GroupController extends Controller {
    ...
    public function get_list(Request $request) {
        ...
        if (isset($data) && !empty($data)) {
            if (isset($data['a']) && !empty($data['a'])) {
                switch ($data['a']) {
                    case 1:
                        return $this->Tbl_a_uac_groups_p_en->__get_tree_view_list($request);
                        break;
                    case 2:
    ...
    public function __get_list_default($request) {
        ...
        $data = $this->Tbl_a_uac_groups_p_en->__find($request, 'all', $params);
            ...
            foreach ($data['data'] AS $keyword => $value) {
                ...
                $arrData[] = [
                    ...
                    'action' => '<div class="btn-group">
                        <button type="button" class="btn btn-sm blue"><a href="' . config('app.base_extraweb_uri') . '/master/uac/groups/edit/' . $this->Converter->base64_basic($value->id, 'encode', ['rep' => 3]) . '" style="color:#fff;font-size:14px;" title="Edit"><i class="fa fa-edit"></i></a></button>
                        <button type="button" class="btn btn-sm yellow"><a href="' . config('app.base_extraweb_uri') . '/master/uac/groups/remove/' . $this->Converter->base64_basic($value->id, 'encode', ['rep' => 3]) . '" style="color:#fff;font-size:14px;" title="Remove"><i class="fa fa-minus-square"></i></a></button>
                        <button type="button" class="btn btn-sm red"><a href="' . config('app.base_extraweb_uri') . '/master/uac/groups/delete/' . $this->Converter->base64_basic($value->id, 'encode', ['rep' => 3]) . '" style="color:#fff;font-size:14px;" title="Delete"><i class="fa fa-trash-o"></i></a></button>
                      </div>',
                ];
    ...
    public function __get_data_by_id($request) {
        ...
        if ($data['id'] == 0) {
            ...
            $dataDB = $this->Tbl_a_uac_groups_p_en->__find($request, 'first', $params);
        ...
        $_config = [
            ...
            'header' => [
                ...
                [
                    'title' => 'List View',
                    'icon' => '<i class="fa fa-list"></i>',
                    'link' => config('app.base_extraweb_uri') . '/master/uac/groups/view'
                ]
            ],
            'tables' => [
                'el-id' => 'dt_tbl_menus',
    ...
    public function __insert_default($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_a_uac_groups_p_en->__find($request, 'first', $params);
            if (isset($response['data']) && !empty($response['data']) && $response['data'] != null) {
                ...
                $this->Tbl_a_uac_groups_p_en->__insert($request, $insert);
    ...
    public function __insert_update_menu_tree($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            if ($data['parent_id'] == 0) {
                ...
                $parent_menu = $this->Tbl_a_uac_groups_p_en->__find($request, 'first', $params);
                if (isset($parent_menu['data']) && !empty($parent_menu['data'])) {
                    $parent_id = $parent_menu['data']->id;
                    $new_level = (int) $parent_menu['data']->__level + 1;

                    $last_rank = $this->Tbl_a_uac_groups_p_en->__fetch_last_rank($request, $parent_id);
                    $new_rank = 1;
            ...
            $response = $this->Tbl_a_uac_groups_p_en->__insert($request, $param_insert);
            if ($response) {
                return $this->General->_set_response('json', ['code' => 200, 'message' => 'successfully insert data', 'valid' => true]);
            }
        }
    ...
    public function edit(Request $request, $params = null) {
        ...
        $_config = [
            ...
            'pages' => [
                ...
                'icon' => '<i class="fa fa-list"></i>',
                'link' => config('app.base_extraweb_uri') . '/master/uac/groups/edit/' . $params
            ],
            'header' => [
                [
                    'title' => 'List View',
        ...
        $menu = $this->Tbl_a_uac_groups_p_en->__find($request, 'first', $params);
        $this->load_css([
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/bootstrap-select/bootstrap-select.min.css",
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/select2/select2.css",
            config('app.base_url_assets_templates') . "/metronic/assets/global/plugins/jquery-multi-select/css/multi-select.css"
    ...
    public function update(Request $request, $params = null) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_a_uac_groups_p_en->__update($request, $update_data, $paramsUpdate);
            if ($response) {
    ...
    public function delete(Request $request, $params = null) {
        if ($params != null) {
            ...
            if ($existData && $existData['data']) {
                ...
                $this->Tbl_a_uac_groups_p_en->__insert($request, $insertUserGroupsBackup, 'mysql_bak');
                $deleteParams = [
                    'table_name' => 'tbl_a_uac_menus_p',
                    'conditions' => [
                        'keyword' => 'id',
