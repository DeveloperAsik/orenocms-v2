...
/**
 * Description of MenupermissionsController
 *
 * @author 64146
 */
class MenupermissionsController {
    ...
    public function view(Request $request) {
        ...
        $_config = [
            'title_for_header' => '<b>Menu Permission</b> master data management page',
            'pages' => [
                'title' => 'View Page Master Data Menu Permissions',
                'icon' => '<i class="fa fa-list"></i>',
                'link' => config('app.base_extraweb_uri') . '/prefferences/uac/menu-permissions/create'
            ],
            'header' => [
                'title' => 'Create',
    ...
    public function __get_list_default($request) {
        ...
        $data = $this->Tbl_b_uac_menu_permissions_r_en->__find($request, 'all', $params);
            ...
            foreach ($data['data'] AS $keyword => $value) {
                ...
                $arrData[] = [
                    ...
                    'action' => '<div class="btn-group">
                        <button type="button" class="btn btn-sm blue"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/menu-permissions/edit/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Edit"><i class="fa fa-edit"></i></a></button>
                        <button type="button" class="btn btn-sm yellow"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/menu-permissions/remove/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Remove"><i class="fa fa-minus-square"></i></a></button>
                        <button type="button" class="btn btn-sm red"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/menu-permissions/delete/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Delete"><i class="fa fa-trash-o"></i></a></button>
                      </div>',
                ];
    ...
    public function update(Request $request, $params = null) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_b_uac_menu_permissions_r_en->__update($request, $update_data, $paramsUpdate);
            if ($response) {
    ...
    public function delete(Request $request, $params = null) {
        if ($params != null) {
            ...
            $existData = $this->Tbl_b_uac_menu_permissions_r_en->__find($request, 'first', $params);
            if ($existData && $existData['data']) {
                $insertUserMenu = [
                    'table_name' => 'tbl_b_uac_menu_permissions_r',
                    'data' => (array) $existData['data']
                ];
                $this->Tbl_b_uac_user_permissions_r_en->__insert($request, $insertUserMenu, 'mysql_bak');
                $deleteParams = [
                    'table_name' => 'tbl_b_uac_menu_permissions_r',
                    'conditions' => [
                        'keyword' => 'id',
                ...
                $response = $this->Tbl_b_uac_menu_permissions_r_en->__delete($request, $deleteParams, 'mysql');
