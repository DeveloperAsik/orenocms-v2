...
/**
 * Description of UserlocationsController
 *
 * @author 64146
 */
class UserlocationsController {
    ...
    public function view(Request $request) {
        ...
        $_config = [
            ...
            'pages' => [
                ...
                'link' => config('app.base_extraweb_uri') . '/prefferences/uac/user-locations/create'
            ],
            'header' => [
                'title' => 'Create',
    ...
    public function __get_list_default($request) {
        ...
        $data = $this->Tbl_a_uac_user_locations_p_en->__find($request, 'all', $params);
            ...
            foreach ($data['data'] AS $keyword => $value) {
                ...
                $arrData[] = [
                    ...
                    'action' => '<div class="btn-group">
                        <button type="button" class="btn btn-sm blue"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/user-locations/edit/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Edit"><i class="fa fa-edit"></i></a></button>
                        <button type="button" class="btn btn-sm yellow"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/user-locations/remove/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Remove"><i class="fa fa-minus-square"></i></a></button>
                        <button type="button" class="btn btn-sm red"><a href="' . config('app.base_extraweb_uri') . '/prefferences/uac/user-locations/delete/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Delete"><i class="fa fa-trash-o"></i></a></button>
                      </div>',
                ];
    ...
    public function update(Request $request, $params = null) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            $response = $this->Tbl_a_uac_user_locations_p_en->__update($request, $update_data, $paramsUpdate);
            if ($response) {
    ...
    public function delete(Request $request, $params = null) {
        if ($params != null) {
            ...
            $existData = $this->Tbl_a_uac_user_locations_p_en->__find($request, 'first', $params);
            if ($existData && $existData['data']) {
                ...
                $response = $this->Tbl_a_uac_user_locations_p_en->__delete($request, $deleteParams, 'mysql');
