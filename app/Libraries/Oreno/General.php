...
class General {
    ...
    public function redirect($path = null) { if ($path != null) { header("Location: " . $path); } }
    ...
    public function validate_img_exist($external_link) { if ($external_link && @getimagesize($external_link)) { return true; } else { return false; }  }
    public function format_money($number) { $result = "Rp " . number_format($number, 2, ',', '.'); return $result; }
    public function slug($string = null) { if($string!=null){$string_1=trim($string);$string_2=strtolower($string_1);$string_3=str_replace(' ','-',$string_2);$string_4=str_replace('--','-',$string_3);$string_5=str_replace('_','-',$string_4);$string_6=str_replace('__','-',$string_5);$string_7=str_replace('/','-',$string_6);return $string_7;} }
    public function url($atRoot = FALSE, $atCore = FALSE, $parse = FALSE) { if(isset($_SERVER['HTTP_HOST'])){$http=isset($_SERVER['HTTPS'])&&strtolower($_SERVER['HTTPS'])!=='off'?'https':'http';$hostname=$_SERVER['HTTP_HOST'];$dir=str_replace(basename($_SERVER['SCRIPT_NAME']),'',$_SERVER['SCRIPT_NAME']);$core=preg_split('@/@',str_replace($_SERVER['DOCUMENT_ROOT'],'',realpath(dirname(__FILE__))),NULL,PREG_SPLIT_NO_EMPTY);$core=$core[0];$tmplt=$atRoot?($atCore?"%s://%s/%s":"%s://%s"):($atCore?"%s://%s/%s/":"%s://%s%s");$end=$atRoot?($atCore?$core:$hostname):($atCore?$core:$dir);if($end=="/"){$base_url=sprintf($tmplt,$http,$hostname);}else{$base_url=sprintf($tmplt,$http,$hostname,$end);}}else{$base_url='http://localhost';}if($parse){$base_url=parse_url($base_url);if(isset($base_url['path'])){if($base_url['path']=='/'){$base_url['path']='';}}}return $base_url; }
    public function session_flash_custom($params = array()) {$message=[];if($params){$message=['alert-msg'=>[['type'=>$params['body']['type'],'message'=>$params['body']['message']]]];session($message);session()->save();} }
}
